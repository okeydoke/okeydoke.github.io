...
/* AU Shop Code - with FPC & Net Price additions */
...
s.linkInternalFilters="javascript:,lululemon.com,lululemon.com.au"
...
s.formList="mailinglist,login,registration,accountsettings,trackOrderbyOrderId,trackOrderbyLastName,shipIt,billingInfo"
...
function s_doPlugins(s) {
	...
	var filtype3=s.getQueryParam('filterType3');

	var filval3=s.getQueryParam('filterValue3');

	if (filtype3)

	var filttemp3=filtype3+"|"+filval3
	...
	s.prop5=s.getTimeParting('h','-8',currentYear); // Set hour of day

	s.prop6=s.getTimeParting('d','-8',currentYear);	// Set Day of week

	s.prop7=s.getTimeParting('w','-5',currentYear); // Set Weekend / Weekday
	...
	if(s.events == "prodView,event3"){

	s.events="prodView,event3,event32";

	}



	if(s.events == "scCheckout"){

	s.events="scCheckout,event33";

	}



	if(s.events == "event15,event7,scCheckout"){

	s.events="event15,event7,scCheckout,event33";

	}

	
	if (typeof s.events != 'undefined') {
	if(s.events.indexOf("purchase,event11") != -1)

	{

		var _flag = false, _tmpEvent11, _tmpNetPrice, _tmpStr;



		s.events = s.apl(s.events,"event8",",",1);

		var _tmpProducts = new Array;

		var _tmpProdDetails = new Array("","","","");

		_tmpProducts = s.split(s.products,",");

		s.products = "";

		for(var i=0; i<_tmpProducts.length; i++)

		{

			if(_tmpProducts[i].indexOf('event11') < 0)  // event11 is not found

			{

				s.products = s.apl(s.products,_tmpProducts[i],",",1);

				_tmpProdDetails = s.split(_tmpProducts[i],";");

				_tmpNetPrice = ( _tmpProdDetails[3] / 110); 

				_tmpNetPrice = Math.round(_tmpNetPrice*10000)/100;

				_tmpStr = "event8=" + _tmpNetPrice;

				s.products = s.apl(s.products, _tmpStr, ";", 1);

			}

			else // event11 is found

			{

				_tmpProdDetails = s.split(_tmpProducts[i],";");

				var _tmpArray = s.split(_tmpProdDetails[1],"=");

				_tmpEvent11 = _tmpArray[1];
...
s.trackingServer="metrics.lululemon.com.au"

s.trackingServerSecure="smetrics.lululemon.com.au"
